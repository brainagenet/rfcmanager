...
public class ChangeRequestParser {
	...
	public void parse(File formFile) throws JXLException, IOException {
		...
		try {
			workbook = Workbook.getWorkbook(new BufferedInputStream(
					new FileInputStream(formFile)));
	...
	private void loadInformation(Sheet sheet) {
		// load component
		String component = getCellContent(sheet,
				ChangeRequestFormInfo.COLUMN_COMPONENT,
		...
		// load module
		String module = getCellContent(sheet,
				ChangeRequestFormInfo.COLUMN_MODULE,
		...
		// load request date
		String reqDate = getCellContent(sheet,
				ChangeRequestFormInfo.COLUMN_REQDATE,
				ChangeRequestFormInfo.ROW_REQDATE);
		changeRequest.setRequestDate(reqDate);

		// load requester
		String requester = getCellContent(sheet,
				ChangeRequestFormInfo.COLUMN_REQUESTER,
				ChangeRequestFormInfo.ROW_REQUESTER);
		changeRequest.setRequester(requester);

		// load summary
		String summary = getCellContent(sheet,
				ChangeRequestFormInfo.COLUMN_SUMMARY,
	...
	private void loadResourceItems(Sheet sheet) {
		...
		for (int row = startRow; row < lastRow; row++) {
			String resource = getCellContent(sheet,
					ChangeRequestFormInfo.COLUMN_RESOURCE, row);
			if (resource == null || resource.length() == 0) {
				break;
			}
			ChangeRequestResource res = new ChangeRequestResource();
			String cellValue;
			res.setNo(Integer.parseInt(getCellContent(sheet,
					ChangeRequestFormInfo.COLUMN_NO, row)));
			res.setResource(resource);
			cellValue = getCellContent(sheet,
					ChangeRequestFormInfo.COLUMN_REVISION, row);
			if (StringUtils.hasText(cellValue)) {
				res.setRevision(Long.parseLong(cellValue));
			}

			cellValue = getCellContent(sheet,
					ChangeRequestFormInfo.COLUMN_CRTYPE, row);
			if (StringUtils.hasText(cellValue)) {
